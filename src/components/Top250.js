...
class Top250 extends Component {
    ...
    render() { 
        return(
            <div>
                {this.renderList()}
                <Pagination defaultCurrent={1} pageSize={this.state.count} total={this.state.totalpage} showTotal={this.showTotal} onChange={this.onChange.bind(this)} />
            </div>
        )
    }
    showTotal(total){
    ...
    getData(){
        ...
        axios.get(`/api/movie/top250?start=${that.state.start}&count=${that.state.count}`).then((res)=>{
            that.setState({
